...
void daxpy(const double a, std::vector<double> const &vec_x, std::vector<double> &vec_y)
{
    assert(vec_x.size() == vec_y.size()); 

    std::transform(std::execution::par_unseq,vec_x.begin(), vec_x.end(),
                  vec_y.begin(), vec_y.begin(),
